...
#
proc ::md5::Chunk {token channel {chunksize 4096} {cmd {}}} {
    upvar #0 $token state
    
    set data [read $channel $chunksize]
    MD5Update $token $data
...
# Process the -command option with some cleanup of local state first.
#
proc ::md5::Finish {tok toclose args} {
    set r [MD5Final $tok]
...
proc ::md5::md5 {args} {
    array set opts {-hex 0 -filename {} -channel {} -chunksize 4096 -command {}}
    ...
    if {$opts(-channel) == {}} {
        ...
        if {$opts(-command) ne {}} {
            set opts(-command) \
                [linsert $opts(-command) 0 [namespace origin Finish] $tok \
                     [expr {$opts(-filename) ne {} ? $opts(-channel) : {}}]]
        }
        # FRINK: nocheck
        set [subst $tok](reading) 1
        fileevent $opts(-channel) readable \
            [list [namespace origin Chunk] \
                 $tok $opts(-channel) $opts(-chunksize) $opts(-command)]
        if {$opts(-command) ne {}} {
            return
        }
        ...
        # If we opened the channel - we will need to close it.
        if {$opts(-filename) != {}} {
            close $opts(-channel)
        }
    }
