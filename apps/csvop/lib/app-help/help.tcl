...
proc ::app-help::run {argv} {
    if {([llength $argv] != 2) && ([llength $argv] != 1) && ([llength $argv] != 0)} {
	...
	tools::usage "
	[tools::appname] help topic ?helpformat?

	    where _topic_ is one of

[ListTopics]

	    and _helpformat_ one of

	        nroff, html, tmml, latex, text, wiki, or list

            If no helpformat is specified it defaults to 'text'.
	"
    }

    foreach {topic format} $argv break
    if {[string equal $format {}]} {set format text}
